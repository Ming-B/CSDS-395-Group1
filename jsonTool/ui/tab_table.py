...
class RangeTreeView(QTreeView):
    ...
    def __init__(self, parent=None):
        ...
        # 当前选中为蓝色（样式表控制）
        self.setStyleSheet(
            "QTreeView::item:selected { background: #cfe8ff; color: black; }"
        )
...
class TableTab(QWidget):
    ...
    def on_document_changed(self, data):
        ...
        # 在加载新文件前，根据需要提示是否清理旧表格视图
        self._maybe_clear_table_after_file_switch()
    ...
    def _maybe_clear_table_after_file_switch(self):
        """在切换 JSON 文件时，如右侧表格有内容，则弹窗询问是否清理。"""
        # 如果表格本身是空的，就什么都不做
        ...
        msg.setText(
            ...
            "Do you want to clear the existing table view generated from the previous file?"
        )
        ...
        # 如果选择 Clear，则清空当前表格视图
        if msg.clickedButton() is btn_clear:
            # 使用已有的列清空逻辑（注意你之前已经把它改成会同时清空行）
            self._delete_all_columns()
    ...
    def _action_confirm_selected_attributes(self):
        ...
        if not sel:
            QMessageBox.information(
                self,
                "Info",
                "Please select one or more attributes on the left JSON view.",
            )
            return
    ...
    def _action_add_to_table(self):
        ...
        if (
            ...
            QMessageBox.information(
                self, "Info", "Please confirm attributes first (Selected Attribute)."
            )
            return
    ...
    # -----表格填充-----
    def _ensure_columns(
        self, headers: List[str], relpaths: List[List[Any]]
    ) -> Dict[int, List[Any]]:
        """确保表格包含指定列；返回 列索引 -> 相对路径 的映射。"""
        existing = {
            self.table.horizontalHeaderItem(i).text(): i
    ...
    def _append_row_by_relpaths(
        self, data_root: Any, base_path: List[Any], col_map: Dict[int, List[Any]]
    ):
    ...
    def _on_header_context_menu(self, pos: QPoint):
        ...
        act_move_rightmost.triggered.connect(
            lambda: self._move_column(col, self.table.columnCount() - 1)
        )
    ...
    def _split_one_level(self, col: int):
        ...
        if not is_any:
            QMessageBox.information(
                self, "Info", "Nothing to split: the column is not JSON objects/arrays."
            )
            return
        ...
        new_cols_info: List[Tuple[str, str]] = (
            []
        )  # (header, kind: "dict:key" / "list:index")
