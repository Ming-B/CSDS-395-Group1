...
from PySide6.QtWidgets import (
    QWidget, QVBoxLayout, QHBoxLayout, QTreeView, QHeaderView, QPushButton, QMessageBox, QApplication
)
...
class ViewerTab(QWidget):
    """Viewer tab: read-only tree with local expand/collapse toolbar and view-state preservation."""
    ...
    def __init__(self, parent=None, document: JSONDocument | None = None):
        ...
        # Preserve/restore view state around model resets (for snapshot jumps)
        ...
        if self.document is not None:
            if self.document.get_data() is not None:
                ...
                # no forced expand; snapshot meta will drive restoration if present
